...
- (instancetype)init
{
    ...
    if (self) {
        ...
        NSArray *faceArray = [NSArray arrayWithContentsOfFile:[KBFaceManager defaultEmojiFacePath]];
...
+ (NSArray *)emojiFaces{
    return [[KBFaceManager shareInstance] emojiFaceArrays];
}
...
+ (NSString *)faceImageNameWithFaceID:(NSUInteger)faceID{
    ...
    for (NSDictionary *faceDict in [[KBFaceManager shareInstance] emojiFaceArrays]) {
...
+(NSString *)faceNameWithFaceID:(NSUInteger)faceID
{
    ...
    for (NSDictionary *faceDict in [[KBFaceManager shareInstance] emojiFaceArrays]) {
        if ([[faceDict objectForKey:kFaceIDKey] integerValue] == faceID) {
            return [faceDict objectForKey:kFaceNameKey];
        }
    }
    return @"";
}
+(NSMutableAttributedString *)emotionStrWithString:(NSString *)text
{
    ...
    for(NSTextCheckingResult *match in resultArray) {
        ...
        for (NSDictionary *dict in [[KBFaceManager shareInstance] emojiFaceArrays]) {
 ...
 */
+ (NSArray *)recentFaces{
    return [[KBFaceManager shareInstance] recentFaceArrays];
}


+ (BOOL)saveRecentFace:(NSDictionary *)recentDict{
    for (NSDictionary *dict in [[KBFaceManager shareInstance] recentFaceArrays]) {
        if ([dict[@"face_id"] integerValue] == [recentDict[@"face_id"] integerValue]) {
            NSLog(@"已经存在");
            return NO;
        }
    }
    [[[KBFaceManager shareInstance] recentFaceArrays] insertObject:recentDict atIndex:0];
    if ([[KBFaceManager shareInstance] recentFaceArrays].count > 8) {
        [[[KBFaceManager shareInstance] recentFaceArrays] removeLastObject];
    }
    [[NSUserDefaults standardUserDefaults] setObject:[[KBFaceManager shareInstance] recentFaceArrays] forKey:@"recentFaceArrays"];
    return YES;
}
@end
